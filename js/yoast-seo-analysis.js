	...
	$( document ).ready( function( e ) {
		...
		if ( $( 'body' ).hasClass( 'post-php' ) && typeof YoastSEO !== "undefined" && typeof SCFYoastSEOAnalysis == "undefined" ) {
			...
			SCFYoastSEOAnalysis.prototype.bindListeners = function() {
				...
				$( '#post-body, #edittag' )
					.find(
						'input[type="text"][name^="smart-custom-fields"], textarea[name^="smart-custom-fields"], .smart-cf-field-type-wysiwyg iframe body#tinymce'
					)
					.on(
						'keyup paste cut blur focus change',
						function() {
			...
			SCFYoastSEOAnalysis.prototype.addScfFieldsToContent = function( data ) {
				...
				$( '#post-body, #edittag' )
					.find(
						'input[type="text"][name^="smart-custom-fields"], textarea[name^="smart-custom-fields"]'
					).each( function() {
						scf_content += ' ' + $( this ).val();
					} );

				$( '.smart-cf-field-type-wysiwyg iframe' )
					.contents()
					.find( 'body#tinymce' )
					.each( function() {
				...
				$( '#post-body, #edittag' )
					.find( '.smart-cf-upload-image img' )
					.each( function() {
						scf_content += $( '<img>' ).attr( {
							src: $( this ).attr( 'src' ) || '',
							alt: $( this ).attr( 'alt' ) || ''
						} )[0].outerHTML;
					} );
